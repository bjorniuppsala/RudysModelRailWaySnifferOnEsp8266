...
constexpr byte bitBufSize = 64; // number of slots for bits
...
void ICACHE_RAM_ATTR dcc_rising()
{
	...
	auto elapsed = now - last_rising;
	last_rising = now;
	auto nbs = nextBitSlot(bitBuffHead);
	if (nbs == bitBuffTail) return;
	else {
	  byte bitFound = elapsed < 170;
	  bitBuffer[bitBuffHead] = bitFound;
	  bitBuffHead = nbs;
	}
}
void beginBitDetection() {
  ...
  attachInterrupt(4, dcc_rising, RISING);
}
...
*/
  ...
  if (checksum) { Serial.print("Bad checksum\n"); printPacket(); return;} // Invalid Checksum
