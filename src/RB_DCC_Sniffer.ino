...
constexpr byte bitBufSize = 128; // number of slots for bits
volatile byte bitBuffer[bitBufSize];
volatile byte bitBuffHead = 0;
...
unsigned int last_rising, last_falling;
...
inline byte ICACHE_RAM_ATTR nextBitSlot(byte slot) {
  slot ++;
...
byte getBit() {
  ...
  while(bitBuffHead == bitBuffTail) { }
  auto result = bitBuffer[bitBuffTail];
  bitBuffTail = nextBitSlot(bitBuffTail);
  return result;
}
...
void ICACHE_RAM_ATTR dcc_rising()
{
	if(digitalRead(4)) {
		last_rising = micros();
	} else {
		last_falling = micros();
		auto elapsed = last_falling - last_rising;
		auto nbs = nextBitSlot(bitBuffHead);
		if (nbs == bitBuffTail) return;
		else {
		  byte bitFound = elapsed < 70;
		  bitBuffer[bitBuffHead] = bitFound;
		  bitBuffHead = nbs;
		}
	}
...
void beginBitDetection() {
  //TCCR0A &= B11111100;
  pinMode(4, INPUT_PULLUP);  // pin D2
  attachInterrupt(4, dcc_rising, CHANGE);
  //attachInterrupt(4, dcc_falling, FALLING);
}
...
*/
  ...
  if (checksum) {
 	Serial.printf("Bad checksum! length = %d\n",pktByteCount);
	printPacket();
	return;
  } // Invalid Checksum
